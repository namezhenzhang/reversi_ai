        ...
        with tqdm(total=len(dataloader)) as t:
            for step, batch in enumerate(dataloader):
                ...
                if (step+1)%(tmp_len*put_len) == 0:
                    loss_plt_num+=1
                    plt.plot(loss_list)
                    plt.savefig('pic/cross/loss{}.jpg'.format(loss_plt_num))

    loss_plt_num+=1
    plt.plot(loss_list)
    plt.savefig('pic/cross/loss{}.jpg'.format(loss_plt_num))

    with open('pic/cross/loss.json','w') as file_obj:
...
batch_size = 1
max_epochs = 1
data_save_path = 'param/3/param128_2h_2layer_40636_cross.pth'
